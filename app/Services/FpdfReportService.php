...
class FpdfReportService
{
    ...
    /**
     * Genera listado general de dependencias (formato vertical).
     */
    public function downloadDependenciasListado(
        ...
        $depsArray = $dependencias instanceof \Illuminate\Support\Collection
            ? $dependencias->all()
        ...
        $widths = [25, 60, 50, 30];
        $headers = ['Código', 'Nombre', 'Unidad Administradora', 'Responsable'];
        ...
        $pdf->SetFont('Arial', '', 8);
        $total = count($depsArray);

        foreach ($depsArray as $dep) {
            $pdf->Cell($widths[0], 6, $this->t((string)($dep->codigo ?? '')), 1, 0, 'C');
            $pdf->Cell($widths[1], 6, $this->t($this->truncate($dep->nombre ?? '', 40)), 1);
            $pdf->Cell($widths[2], 6, $this->t($dep->unidadAdministradora?->nombre ?? 'N/A'), 1);
            $pdf->Cell($widths[3], 6, $this->t($dep->responsable?->nombre_completo ?? 'N/A'), 1, 0, 'C');
            $pdf->Ln();
        }

        if ($total === 0) {
            $pdf->Cell(array_sum($widths), 10, $this->t('No se encontraron dependencias.'), 1, 1, 'C');
        }
    ...
    /**
     * Genera reporte agrupado de dependencias por unidad administradora.
     */
    public function generarDependenciasPorUnidad(string $fileName, string $title, ?string $subtitle, string $generatedAt, iterable $dependencias)
    {
        $pdf = $this->make('L');
        $depsArray = $dependencias instanceof \Illuminate\Support\Collection
            ? $dependencias->all()
        ...
        $agrupados = [];
        foreach ($depsArray as $dep) {
            $uniNombre = optional($dep->unidadAdministradora)->nombre ?? 'Sin Unidad';
        ...
        foreach ($agrupados as $uniNombre => $depsGrupo) {
            ...
            $pdf->SetTextColor(255, 255, 255);
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->Cell(0, 8, $this->t('UNIDAD ADMINISTRADORA: '.strtoupper($this->t($uniNombre))), 0, 1, 'L', true);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Ln(2);

            $widths = [20, 70, 40, 30, 50];
            $headers = ['Código', 'Dependencia', 'Responsable', 'Bienes', 'Observaciones'];
            ...
            $pdf->SetFont('Arial', '', 8);
            foreach ($depsGrupo as $dep) {
                $pdf->Cell($widths[0], 6, $this->t($dep->codigo ?? ''), 1, 0, 'C');
                $pdf->Cell($widths[1], 6, $this->t($this->truncate($dep->nombre ?? '', 40)), 1);
                $pdf->Cell($widths[2], 6, $this->t($dep->responsable?->nombre_completo ?? 'N/A'), 1);
                $pdf->Cell($widths[3], 6, (string)($dep->bienes_count ?? 0), 1, 0, 'C');
                $pdf->Cell($widths[4], 6, $this->t($this->truncate($dep->observaciones ?? '', 30)), 1);
                $pdf->Ln();
            }
    ...
    /**
     * Genera reporte agrupado de dependencias por responsable.
     */
    public function generarDependenciasPorResponsable(string $fileName, string $title, ?string $subtitle, string $generatedAt, iterable $dependencias)
    {
        $pdf = $this->make('L');
        $depsArray = $dependencias instanceof \Illuminate\Support\Collection
            ? $dependencias->all()
        ...
        $agrupados = [];
        foreach ($depsArray as $dep) {
            $respNombre = optional($dep->responsable)->nombre_completo ?? 'Sin Responsable';
            $agrupados[$respNombre][] = $dep;
        }

        $this->renderHeader($pdf, $title, $subtitle, $generatedAt, []);

        foreach ($agrupados as $respNombre => $depsGrupo) {
            if ($pdf->GetY() > 150) {
                $pdf->AddPage();
            }
            ...
            $pdf->SetTextColor(255, 255, 255);
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->Cell(0, 8, $this->t('RESPONSABLE: '.strtoupper($this->t($respNombre))), 0, 1, 'L', true);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Ln(2);

            $widths = [20, 70, 60, 30];
            $headers = ['Código', 'Dependencia', 'Unidad', 'Bienes'];
            ...
            foreach ($depsGrupo as $dep) {
                $pdf->Cell($widths[0], 6, $this->t($dep->codigo ?? ''), 1, 0, 'C');
                $pdf->Cell($widths[1], 6, $this->t($this->truncate($dep->nombre ?? '', 40)), 1);
                $pdf->Cell($widths[2], 6, $this->t($this->truncate($dep->unidadAdministradora?->nombre ?? 'N/A', 35)), 1);
                $pdf->Cell($widths[3], 6, (string)($dep->bienes_count ?? 0), 1, 0, 'C');
                $pdf->Ln();
            }
     ...
     */
    public function downloadUsuariosListado(
        string $fileName,
        string $title,
        ?string $subtitle,
        string $generatedAt,
        ...
        $usersArray = $usuarios instanceof \Illuminate\Support\Collection
            ? $usuarios->all()
            : iterator_to_array($usuarios);

        $pdf = $this->make('P');
        $this->renderHeader($pdf, $title, $subtitle, $generatedAt, []);

        $widths = [20, 70, 60, 50, 25];
        $headers = ['Cédula', 'Nombre Completo', 'Correo', 'Rol', 'Estado'];
        ...
        foreach ($usersArray as $usuario) {
            $nombreCompleto = trim(($usuario->nombre ?? '') . ' ' . ($usuario->apellido ?? ''));
            $pdf->Cell($widths[0], 6, $this->t((string)($usuario->cedula ?? '')), 1, 0, 'C');
            $pdf->Cell($widths[1], 6, $this->t($this->truncate($nombreCompleto, 42)), 1);
            $pdf->Cell($widths[2], 6, $this->t($this->truncate($usuario->correo ?? '', 38)), 1);
            $pdf->Cell($widths[3], 6, $this->t($usuario->rol?->nombre ?? 'Sin rol'), 1);
            $estado = $usuario->activo ? 'Activo' : 'Inactivo';
            $pdf->Cell($widths[4], 6, $this->t($estado), 1, 0, 'C');
            $pdf->Ln();
        }

        if ($total === 0) {
            $pdf->Cell(array_sum($widths), 10, $this->t('No se encontraron usuarios.'), 1, 1, 'C');
        }
     ...
     */
    public function generarUsuariosPorRol(string $fileName, string $title, ?string $subtitle, string $generatedAt, iterable $usuarios)
    {
        $pdf = $this->make('L');
        $usersArray = $usuarios instanceof \Illuminate\Support\Collection
            ? $usuarios->all()
        ...
        $withRol = collect($usersArray)->filter(fn($u) => $u->rol !== null);
        $agrupados = $withRol->groupBy(fn($u) => $u->rol?->nombre ?? 'Sin Rol')->toArray();
        $sinRol = $withRol->count() < count($usersArray);
        if ($sinRol) {
            $agrupados['Sin Rol'] = collect($usersArray)->filter(fn($u) => $u->rol === null)->all();
        }

        $this->renderHeader($pdf, $title, $subtitle, $generatedAt, []);

        foreach ($agrupados as $rolNombre => $usersGrupo) {
            if ($pdf->GetY() > 150) {
                $pdf->AddPage();
            }
            ...
            $pdf->SetTextColor(255, 255, 255);
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->Cell(0, 8, $this->t('ROL: '.strtoupper($this->t($rolNombre))), 0, 1, 'L', true);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Ln(2);

            $widths = [20, 70, 60, 25];
            $headers = ['Cédula', 'Nombre Completo', 'Correo', 'Estado'];
            ...
            $pdf->SetFont('Arial', '', 8);
            foreach ($usersGrupo as $usuario) {
                $nombreCompleto = trim(($usuario->nombre ?? '') . ' ' . ($usuario->apellido ?? ''));
                $pdf->Cell($widths[0], 6, $this->t((string)($usuario->cedula ?? '')), 1, 0, 'C');
                $pdf->Cell($widths[1], 6, $this->t($this->truncate($nombreCompleto, 42)), 1);
                $pdf->Cell($widths[2], 6, $this->t($this->truncate($usuario->correo ?? '', 38)), 1);
                $estado = $usuario->activo ? 'Activo' : 'Inactivo';
                $pdf->Cell($widths[3], 6, $this->t($estado), 1, 0, 'C');
                $pdf->Ln();
            }
    ...
    public function downloadOrganismosListado(
        ...
        $orgsArray = $organismos instanceof \Illuminate\Support\Collection
            ? $organismos->all()
        ...
        $widths = [25, 70, 50];
        $headers = ['Código', 'Nombre del Organismo', 'Unidades'];
        ...
        $pdf->SetFont('Arial', '', 8);
        $total = count($orgsArray);

        foreach ($orgsArray as $org) {
            $unidadesCount = $org->unidadesAdministradoras_count ?? $org->unidadesAdministradoras?->count() ?? 0;
            $pdf->Cell($widths[0], 6, $this->t((string)($org->codigo ?? '')), 1, 0, 'C');
            $pdf->Cell($widths[1], 6, $this->t($this->truncate($org->nombre ?? '', 50)), 1);
            $pdf->Cell($widths[2], 6, (string)$unidadesCount, 1, 0, 'C');
            $pdf->Ln();
        }

        if ($total === 0) {
            $pdf->Cell(array_sum($widths), 10, $this->t('No se encontraron organismos.'), 1, 1, 'C');
        }
    ...
    /**
     * Listado general de unidades administradoras.
     */
    public function downloadUnidadesListado(
        ...
        $unidsArray = $unidades instanceof \Illuminate\Support\Collection
            ? $unidades->all()
        ...
        $widths = [25, 60, 60, 50];
        $headers = ['Código', 'Nombre', 'Organismo', 'Dependencias'];
        ...
        $pdf->SetFont('Arial', '', 8);
        $total = count($unidsArray);

        foreach ($unidsArray as $unidad) {
            $depsCount = $unidad->dependencias_count ?? $unidad->dependencias?->count() ?? 0;
            $pdf->Cell($widths[0], 6, $this->t((string)($unidad->codigo ?? '')), 1, 0, 'C');
            $pdf->Cell($widths[1], 6, $this->t($this->truncate($unidad->nombre ?? '', 35)), 1);
            $pdf->Cell($widths[2], 6, $this->t($this->truncate($unidad->organismo?->nombre ?? 'N/A', 35)), 1);
            $pdf->Cell($widths[3], 6, (string)$depsCount, 1, 0, 'C');
            $pdf->Ln();
        }

        if ($total === 0) {
            $pdf->Cell(array_sum($widths), 10, $this->t('No se encontraron unidades administradoras.'), 1, 1, 'C');
        }
    ...
    /**
     * Unidades agrupadas por organismo.
     */
    public function generarUnidadesPorOrganismo(string $fileName, string $title, ?string $subtitle, string $generatedAt, iterable $unidades)
    {
        $pdf = $this->make('L');
        $unidsArray = $unidades instanceof \Illuminate\Support\Collection
            ? $unidades->all()
        ...
        $agrupados = [];
        foreach ($unidsArray as $unidad) {
            $orgNombre = optional($unidad->organismo)->nombre ?? 'Sin Organismo';
            $agrupados[$orgNombre][] = $unidad;
        }

        $this->renderHeader($pdf, $title, $subtitle, $generatedAt, []);

        foreach ($agrupados as $orgNombre => $unidsGrupo) {
            if ($pdf->GetY() > 150) {
                $pdf->AddPage();
            }
            ...
            $pdf->SetTextColor(255, 255, 255);
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->Cell(0, 8, $this->t('ORGANISMO: '.strtoupper($this->t($orgNombre))), 0, 1, 'L', true);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Ln(2);

            $widths = [20, 60, 30, 50];
            $headers = ['Código', 'Unidad Administradora', 'Dependencias', 'Responsable'];
            ...
            $pdf->SetFont('Arial', '', 8);
            foreach ($unidsGrupo as $unidad) {
                $depsCount = $unidad->dependencias_count ?? $unidad->dependencias?->count() ?? 0;
                $pdf->Cell($widths[0], 6, $this->t((string)($unidad->codigo ?? '')), 1, 0, 'C');
                $pdf->Cell($widths[1], 6, $this->t($this->truncate($unidad->nombre ?? '', 40)), 1);
                $pdf->Cell($widths[2], 6, (string)$depsCount, 1, 0, 'C');
                $pdf->Cell($widths[3], 6, $this->t($this->truncate($unidad->responsable?->nombre_completo ?? 'N/A', 30)), 1);
                $pdf->Ln();
            }
    ...
    public function downloadMovimientosListado(
        ...
        $movesArray = $movimientos instanceof \Illuminate\Support\Collection
            ? $movimientos->all()
        ...
        $widths = [25, 30, 35, 45, 80];
        $headers = ['Fecha', 'Tipo', 'Usuario', 'Entidad', 'Observaciones'];
        ...
        $pdf->SetFont('Arial', '', 8);
        $total = count($movesArray);

        foreach ($movesArray as $mov) {
            $pdf->Cell($widths[0], 6, $mov->fecha?->format('d/m/Y') ?? '-', 1, 0, 'C');
            $pdf->Cell($widths[1], 6, $this->t($this->truncate($mov->tipo ?? '-', 12)), 1, 0, 'C');
            $pdf->Cell($widths[2], 6, $this->t($this->truncate($mov->usuario?->nombre_completo ?? 'N/A', 20)), 1, 0, 'C');
            $entidad = class_basename($mov->subject_type ?? '') . ' #' . ($mov->subject_id ?? '?');
            $pdf->Cell($widths[3], 6, $this->t($this->truncate($entidad, 25)), 1, 0, 'C');
            $pdf->Cell($widths[4], 6, $this->t($this->truncate($mov->descripcion ?? $mov->observaciones ?? '', 50)), 1);
            $pdf->Ln();
        }
     ...
     */
    public function generarMovimientosPorTipo(string $fileName, string $title, ?string $subtitle, string $generatedAt, iterable $movimientos)
    {
        $pdf = $this->make('L');
        $movesArray = $movimientos instanceof \Illuminate\Support\Collection
            ? $movimientos->all()
        ...
        $agrupados = collect($movesArray)->groupBy('tipo')->toArray();

        $this->renderHeader($pdf, $title, $subtitle, $generatedAt, []);

        foreach ($agrupados as $tipo => $movesGrupo) {
            if ($pdf->GetY() > 150) {
                $pdf->AddPage();
            }
            ...
            $pdf->SetTextColor(255, 255, 255);
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->Cell(0, 8, $this->t('TIPO: '.strtoupper($this->t($tipo))), 0, 1, 'L', true);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Ln(2);

            $widths = [25, 30, 50, 90];
            $headers = ['Fecha', 'Usuario', 'Entidad Afectada', 'Observaciones'];
            ...
            $pdf->SetFont('Arial', '', 8);
            foreach ($movesGrupo as $mov) {
                $pdf->Cell($widths[0], 6, $mov->fecha?->format('d/m/Y') ?? '-', 1, 0, 'C');
                $pdf->Cell($widths[1], 6, $this->t($this->truncate($mov->usuario?->nombre_completo ?? 'N/A', 20)), 1, 0, 'C');
                $entidad = class_basename($mov->subject_type ?? '') . ' #' . ($mov->subject_id ?? '?');
                $pdf->Cell($widths[2], 6, $this->t($this->truncate($entidad, 30)), 1, 0, 'C');
                $pdf->Cell($widths[3], 6, $this->t($this->truncate($mov->descripcion ?? $mov->observaciones ?? '', 60)), 1);
                $pdf->Ln();
            }
     ...
     */
    public function generarMovimientosPorUsuario(string $fileName, string $title, ?string $subtitle, string $generatedAt, iterable $movimientos)
    {
        $pdf = $this->make('L');
        $movesArray = $movimientos instanceof \Illuminate\Support\Collection
            ? $movimientos->all()
        ...
        $withUser = collect($movesArray)->filter(fn($m) => $m->usuario !== null);
        $agrupados = $withUser->groupBy(fn($m) => $m->usuario?->nombre_completo ?? 'Sin Usuario')->toArray();
        $sinUser = $withUser->count() < count($movesArray);
        if ($sinUser) {
            $agrupados['Sin Usuario'] = collect($movesArray)->filter(fn($m) => $m->usuario === null)->all();
        }

        $this->renderHeader($pdf, $title, $subtitle, $generatedAt, []);

        foreach ($agrupados as $usuarioNombre => $movesGrupo) {
            if ($pdf->GetY() > 150) {
                $pdf->AddPage();
            }
            ...
            $pdf->SetTextColor(255, 255, 255);
            $pdf->SetFont('Arial', 'B', 11);
            $pdf->Cell(0, 8, $this->t('USUARIO: '.strtoupper($this->t($usuarioNombre))), 0, 1, 'L', true);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Ln(2);

            $widths = [25, 30, 50, 95];
            $headers = ['Fecha', 'Tipo', 'Entidad Afectada', 'Descripción'];
