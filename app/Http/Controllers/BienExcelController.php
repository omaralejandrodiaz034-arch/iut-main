...
class BienExcelController extends Controller
{
    ...
    public function importar(Request $request)
    {
        ...
        try {
            ...
            if (count($rows) > 1000) {
                return back()->with('error', 'El archivo no puede contener más de 1000 registros.');
            }
    ...
    public function exportar(Request $request)
    {
        try {
            ...
            // Filtrado de Desincorporados: por defecto se excluyen
            $soloDesincorporados = filter_var($request->input('solo_desincorporados', false), FILTER_VALIDATE_BOOLEAN);
            $filtroEstadoExplicito = $request->filled('estado') && $request->estado === 'DESINCORPORADO';
